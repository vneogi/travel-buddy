...
6. party_context is MERGED into value_json (doesn't overwrite existing fields)
7. No birth date fields accepted (age_band only)
"""
...
@pytest.mark.asyncio
async def test_create_trip_with_party():
    """Create trip with explicit family party — persisted and returned."""
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.post("/api/v1/trip/create", json={
            "start_date": _now_iso(),
            "party": {
...
@pytest.mark.asyncio
async def test_create_trip_without_party_defaults_solo():
...
@pytest.mark.asyncio
async def test_get_trip_includes_party():
    """GET /trip/{id} returns the party for display."""
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        # Create a trip with a party
        create_resp = await client.post("/api/v1/trip/create", json={
            "start_date": _now_iso(),
            "party": {
                "party_type": "couple",
                "size": 2,
        ...
        # GET the trip
        get_resp = await client.get(f"/api/v1/trip/{trip_id}", headers=HEADERS)
...
@pytest.mark.asyncio
async def test_signal_gets_party_context_stamped():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        signal_resp = await client.post("/api/v1/signals", json={
            "signals": [{
...
@pytest.mark.asyncio
async def test_signal_unknown_trip_still_succeeds():
...
@pytest.mark.asyncio
async def test_signal_no_trip_id_still_succeeds():
...
@pytest.mark.asyncio
async def test_party_context_merge_preserves_existing_value_json():
...
@pytest.mark.asyncio
async def test_no_birth_date_in_party_schema():
