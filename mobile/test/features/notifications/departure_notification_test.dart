...
void _stubNotificationStorage(_MockOfflineDatabase db) {
  when(() => db.pruneNotificationData()).thenAnswer((_) async {});
  when(() => db.pruneAlertData()).thenAnswer((_) async {});
  when(
    () => db.getDismissedNotificationIds(
      identityScope: any(named: 'identityScope'),
    ),
  ).thenAnswer((_) async => <String>{});
  when(
    () => db.getDismissedAlertIds(identityScope: any(named: 'identityScope')),
  ).thenAnswer((_) async => <String>{});
  when(
    () => db.cacheNotifications(
      identityScope: any(named: 'identityScope'),
...
Map<String, dynamic> _candidateJson({
  ...
  'evidence': {
    ...
    'policy': {'arrival_buffer_minutes': 10, 'weather_buffer_minutes': 10},
  },
};
  ...
  'alerts': [
    {
      ...
      'evidence': {'rain_probability': 0.63, 'condition_code': 500},
      'auto_applied': false,
    },
  ],
  'refreshed_at': '2026-10-04T01:05:00Z',
...
void main() {
  ...
  group('model parsing', () {
    ...
    test('parses candidate without weather evidence', () {
      ...
      final evidence = Map<String, dynamic>.from(j['evidence'] as Map);
  ...
  test('dismissed ID stays hidden after rebuild', () async {
    ...
    final state = DepartureState(candidates: [candidate], dismissed: dismissed);
    expect(state.visible, isNull);
    ...
    await tester.pumpWidget(
      UncontrolledProviderScope(
        ...
        child: const MaterialApp(
          home: Scaffold(body: ItineraryAlertsSection(tripId: 'trip-123')),
        ),
      ),
    );
    final departure = await container.read(
      departureNotifierProvider('trip-123').future,
    );
    await container.read(alertsNotifierProvider('trip-123').future);
    await tester.pump();

    expect(departure.status, 'available');
  ...
  testWidgets('absent departure leaves weather cards visible', (tester) async {
    ...
    final alerts = await container.read(
      alertsNotifierProvider('trip-123').future,
    );
    await tester.pump();

    expect(departure.visible, isNull);
    expect(alerts.visible, hasLength(1));
  ...
  testWidgets('banner text equals server title and message', (tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(body: DepartureBanner(candidate: candidate)),
      ),
    );
    await tester.pump();

    // Title must match server exactly
    expect(find.text('Time to head to Kuang Si Falls'), findsOneWidget);
  ...
  test('notification_dismissals is separate from alert_dismissals', () async {
    ...
    // Dismiss an alert
    await db.dismissAlert(identityScope: 'scope-1', alertId: 'alert-y');
  ...
  testWidgets('resume inside 15 minutes does not issue a second request', (
    ...
    verifyNever(
      () => api.get('/trip/trip-123/notifications', query: any(named: 'query')),
    );
  });

  // ===================== Proof: Tap on known node_id =====================
  testWidgets('tap on banner calls onTapNodeId with correct nodeId', (
    tester,
  ) async {
    String? tappedNodeId;
    final candidate = NotificationCandidate.fromJson(_candidateJson());
  ...
  test(
    'network failure with unexpired cache still shows cached banner',
    () async {
      final db = OfflineDatabase(testPath: inMemoryDatabasePath);
  ...
  test('HTTP 503 with unexpired cache still shows cached banner', () async {
    ...
    final state = await container.read(departureNotifierProvider('t1').future);
    expect(state.status, 'cached');
  ...
  testWidgets('SABOTAGE: client-side message suffix is detected', (
    tester,
  ) async {
    // If someone hard-codes a suffix onto message, this test catches it.
    final candidate = NotificationCandidate.fromJson(
      _candidateJson(message: 'Server message only.'),
    );
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(body: DepartureBanner(candidate: candidate)),
  ...
  test(
    'SABOTAGE: dismiss survives rebuild via notification_dismissals',
    () async {
