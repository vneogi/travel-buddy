...
## SPEC-37: Phone-Independent Field-Test Acceptance

Target: 2026-09-18. Invalid if the app uses flutter run, localhost, 10.0.2.2,
laptop LAN, USB networking, or adb reverse.
...
|---|---|
| Source commit | |
| Branch | feat/spec37-phone-field-test |
| Platform | Android |
...
- [ ] `debug_mode` is false
- [ ] `llm_key_present` is true
- [ ] `supabase_configured` is true
- [ ] `jwt_auth` is false (JWT secret intentionally unset)
- [ ] Startup logs show booleans only, no secret values
- [ ] Anonymous `GET /api/v1/trips` succeeds
- [ ] Anonymous trip read succeeds
- [ ] `GET /api/v1/trip/{id}/notifications` does not return "unconfigured"
- [ ] Server logs contain no debug impersonation mode

### Online acceptance (phone disconnected from laptop)

- [ ] App launches from phone home screen without laptop
- [ ] Device identity is stable across app restart
- [ ] SPEC-36 Laos corridor opens with three city sections
- [ ] Driver card opens for each city
- [ ] Later-city swap: only the targeted city changes
- [ ] Departure notification retrieval succeeds
...
- [ ] Force-kill and reopen: app launches
- [ ] Cached itinerary is visible
- [ ] Each cached city/day section expands
- [ ] Pre-cached driver card shows local name, landmark, coordinates
- [ ] Record an offline-supported signal (e.g., heart)
- [ ] Force-kill and reopen: cached content and pending action remain
- [ ] Restore network: pending action syncs once without duplication
