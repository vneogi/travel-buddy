...
| Booking anchors (SPEC-10) | PARTIAL | Create/edit/delete on main (PR #20, PR #37). Provider-aware paste on main (`fc6926b`). Flight cutoff and hotel origin/return on main (`e7a0457`). Naive wall times are destination-local on main (`933c705`). `pack_day` booking constraints, multi-night UI, dual check-in/out display, hotel coords, and server-side document consent remain |
...
| Trust and verification (SPEC-17) | PARTIAL (disclosure slice) | GET /venues/search returns flat results; the Sep 5 repair derives sponsored_boost_applied from the RPC's positive score delta so Supabase cannot drop the disclosure with private bid fields. Windows evening 5 Sep: live Dubai search returned three boosted rows with both flags true. SwapSheet labels boosted results and explains ranking influence; positive UI observation still missing from that log. Full claims, registry, and staleness remain unimplemented |
...
- **Sponsored-placement disclosure (SPEC-17 decision 15).** SwapSheet now
...
2. SPEC-37 phone-independent field-test delivery -- **DONE** PR #57
...
8. Destination-local booking times and configured-env guards -- **DONE on
   main** (`933c705`, from `1cd7c6a` + `1757daf` + `ca3074f`). Naive ISO is
...
| Swap copy and empty weather key leak through configured `.env` | Closed on main (`933c705`) | Empty constructor key stays unconfigured. Swap canned copy runs after breaker and `no_candidates`, so a failed swap cannot say it applied |
| Trip Chat is wired but not grounded | Closed on main for trip-scoped Ask (`30a4270`); hosted/device unverified | Laptop Flutter envelope tests passed. Hosted composer and phone Ask are not yet re-run on this SHA. Trip-optional Ask remains |
