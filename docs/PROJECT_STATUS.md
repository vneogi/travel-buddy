...
| Laos corridor trip (SPEC-36) | IMPLEMENTATION IN REVIEW, NOT MERGED | `origin/feat/spec36-laos-corridor` is at `f117967`. Production import, scrolling, next-node, and backend proof fixes are pushed. Remaining blockers are production-path Home date validation, effective CitySection state assertions, cross-city `nextNode` widget proof, mutation assertions in boundary tests, direct LLM sabotage, and actual Flutter analyze/test plus CI |
...
1. SPEC-36 Laos corridor trip -- **IN REVIEW, NOT MERGED** at `f117967`.
