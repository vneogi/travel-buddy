...
| Trust and verification (SPEC-17) | PARTIAL (disclosure slice) | GET /venues/search returns flat results; the Sep 5 repair derives sponsored_boost_applied from the RPC's positive score delta so Supabase cannot drop the disclosure with private bid fields. SwapSheet labels boosted results and explains ranking influence. Full claims, registry, and staleness remain unimplemented |
