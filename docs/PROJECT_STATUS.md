...
| Hotel stay can shift local date and collapse to one timestamp | High booking correctness | Sep 18 old APK shifted an entered Oct 2 18:02 Laos check-in to Oct 3 01:02 and hid checkout. Naive ISO is stamped UTC in `state_machine.py`; cards format destination-local. Genie brief: `docs/briefs/GENIE_SPEC_10_BOOKING_LOCAL_TIME.md`. Reproduce on current main; do not APK until this round-trips |
