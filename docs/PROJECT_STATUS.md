...
| Generated schedules expose false precision and morning-heavy days | Medium | Exact walking arithmetic can render starts such as 10:52 and earliest-fit packing can end a nominal full day around lunch. SPEC-41 now requires a forward-rounded human time grid and balanced day windows when feasible, without inventing filler |
| Hotel stay can shift local date and collapse to one timestamp | High booking correctness | Sep 18 old APK shifted an entered Oct 2 18:02 Laos check-in to Oct 3 01:02 and hid checkout. Reproduce on current main; SPEC-10 now requires destination-local round-trip, explicit check-in/out, covered dates, and HITL reflow intent |
